...
print_usage() {
  cat <<'EOF'
Usage: scripts/record_capture_submit_close_trace.sh [options]

Record a live capture submit-close trace by launching Prompt Cue directly,
auto-opening capture with a seeded draft, auto-submitting once the editor
focuses, attaching xctrace to the exact process, and exporting the trace plus
the measured panel-close log.
...
fail() {
  echo "record_capture_submit_close_trace: $*" >&2
  exit 1
}
...
if [[ -z "${OUT_DIR}" ]]; then
  OUT_DIR="/tmp/promptcue-traces/capture-submit-close/${TIMESTAMP}"
fi
mkdir -p "${OUT_DIR}"
...
if [[ -z "${DRAFT_FILE}" ]]; then
  ...
  printf '%s\n' "Capture submit-close trace seed" > "${DRAFT_FILE}"
fi
[[ -f "${DRAFT_FILE}" ]] || fail "draft file does not exist: ${DRAFT_FILE}"

TRACE_PATH="${OUT_DIR}/capture-submit-close.trace"
STDOUT_LOG="${OUT_DIR}/stdout.log"
TOC_PATH="${OUT_DIR}/trace-toc.xml"

echo "Recording live capture submit-close trace..."
APP_PID=""
...
launch_target_app() {
  ...
  PROMPTCUE_TRACE_CAPTURE_SUBMIT_CLOSE_ON_START=1 \
  PROMPTCUE_TRACE_CAPTURE_SUBMIT_CLOSE_DELAY_MS=1500 \
  PROMPTCUE_TRACE_AUTO_QUIT_AFTER_CAPTURE_SUBMIT_CLOSE=1 \
  PROMPTCUE_TRACE_CAPTURE_SUBMIT_CLOSE_STDOUT_METRIC=1 \
  PROMPTCUE_TRACE_CAPTURE_PHASES=1 \
...
while true; do
  ...
  if [[ ${XCTRACE_STATUS} -eq 21 && ${ATTEMPT} -lt ${ATTACH_RETRY_LIMIT} ]]; then
    echo "record_capture_submit_close_trace: attach attempt ${ATTEMPT} failed; retrying..." >&2
    ATTEMPT=$((ATTEMPT + 1))
...
if kill -0 "${APP_PID}" >/dev/null 2>&1; then
  kill "${APP_PID}" >/dev/null 2>&1 || true
  trap - EXIT
  fail "target app did not exit after capture submit-close trace capture; check that the traced build includes PerformanceTrace hooks"
fi
...
METRIC_LINE="$(rg -o 'PROMPTCUE_CAPTURE_SUBMIT_CLOSE_MS=[0-9.]+' "${STDOUT_LOG}" | tail -n 1 || true)"
if [[ -z "${METRIC_LINE}" ]]; then
  fail "no capture submit-close metric found in ${STDOUT_LOG}"
fi
