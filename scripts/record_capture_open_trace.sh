...
print_usage() {
  cat <<'EOF'
Usage: scripts/record_capture_open_trace.sh [options]

Record a live capture-open trace by launching Prompt Cue directly, auto-triggering
the real capture-open path, attaching xctrace to the exact process, and exporting
...
fail() {
  echo "record_capture_open_trace: $*" >&2
  exit 1
}
...
if [[ -z "${OUT_DIR}" ]]; then
  OUT_DIR="/tmp/promptcue-traces/capture-open/${TIMESTAMP}"
fi
mkdir -p "${OUT_DIR}"
OUT_DIR="$(cd "${OUT_DIR}" && pwd)"

TRACE_PATH="${OUT_DIR}/capture-open.trace"
STDOUT_LOG="${OUT_DIR}/stdout.log"
TOC_PATH="${OUT_DIR}/trace-toc.xml"

echo "Recording live capture-open trace..."
APP_PID=""
...
launch_target_app() {
  : >"${STDOUT_LOG}"
  PROMPTCUE_TRACE_CAPTURE_TOGGLE_ON_START=1 \
  PROMPTCUE_TRACE_CAPTURE_TOGGLE_DELAY_MS=1500 \
  PROMPTCUE_TRACE_AUTO_QUIT_AFTER_CAPTURE_OPEN=1 \
  PROMPTCUE_TRACE_CAPTURE_OPEN_STDOUT_METRIC=1 \
...
while true; do
  ...
  if [[ ${XCTRACE_STATUS} -eq 21 && ${ATTEMPT} -lt ${ATTACH_RETRY_LIMIT} ]]; then
    echo "record_capture_open_trace: attach attempt ${ATTEMPT} failed; retrying..." >&2
    ATTEMPT=$((ATTEMPT + 1))
...
if kill -0 "${APP_PID}" >/dev/null 2>&1; then
  kill "${APP_PID}" >/dev/null 2>&1 || true
  trap - EXIT
  fail "target app did not exit after capture-open trace capture; check that the traced build includes PerformanceTrace hooks"
fi
...
METRIC_LINE="$(rg -o 'PROMPTCUE_CAPTURE_OPEN_FOCUSED_EDITOR_MS=[0-9.]+' "${STDOUT_LOG}" | tail -n 1 || true)"
if [[ -z "${METRIC_LINE}" ]]; then
  fail "no capture-open metric found in ${STDOUT_LOG}"
fi
