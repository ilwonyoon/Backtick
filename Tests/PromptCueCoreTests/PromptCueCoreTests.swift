...
struct PromptCueCoreTests {
    ...
    @Test
    func workItemSanitizesFieldsAndClampsSourceCount() {
        let item = WorkItem(
            title: "  Fix selector drift  ",
            summary: "  Review stack reassignment edge cases  ",
            repoName: "  PromptCue  ",
            branchName: "  feat/backtick-mcp  ",
        ...
        #expect(item.title == "Fix selector drift")
        #expect(item.summary == "Review stack reassignment edge cases")
        #expect(item.repoName == "PromptCue")
        #expect(item.branchName == "feat/backtick-mcp")
        #expect(item.status == .open)
        #expect(item.updatedAt == referenceDate)
        #expect(item.sourceNoteCount == 0)
        #expect(item.isResolved == false)
    }

    @Test
    func workItemUpdatingStatusRefreshesUpdatedAt() {
        let createdAt = Date(timeIntervalSince1970: 1_000)
        let updatedAt = Date(timeIntervalSince1970: 2_000)
        let original = WorkItem(
            title: "Stabilize MCP contract",
            createdAt: createdAt,
            createdBy: .user,
            sourceNoteCount: 2
        )

        let updated = original.updatingStatus(.done, updatedAt: updatedAt)
        ...
        #expect(updated.isResolved)
    }

    @Test
    func workItemJSONCodecRoundTrip() throws {
        let original = WorkItem(
            id: UUID(),
            title: "Design MCP board",
            summary: "Lock repo and status lane semantics",
            repoName: "PromptCue",
    ...
    @Test
    func workItemSourceJSONCodecRoundTrip() throws {
        let original = WorkItemSource(
            workItemID: UUID(),
            noteID: UUID(),
        ...
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(WorkItemSource.self, from: data)
